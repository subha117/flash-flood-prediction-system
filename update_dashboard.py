  ...
  const fetchData = async (lat, lng) => {
    ...
    try {
      ...
      if (locRes.ok) {
        ...
        setLocationInfo({
          name: locData.location_name || "Unknown",
      ...
      const predBody = {
        latitude: lat,
        longitude: lng,
        elevation_m: featData.elevation_m || 0,
        slope_degree: featData.slope_degree || 0,
      ...
      if (predRes.ok) {
        ...
        savedHist.unshift({
          ...
          location: locRes.ok ? locationInfo.name : "Unknown",
        ...
        <main className="dashboard-content">
          <section className="top-stat-grid">
            <div className="stat-card flood-risk-card">
              <div className={`stat-icon ${prediction?.risk_level === 'HIGH' ? 'red' : prediction?.risk_level === 'MEDIUM' ? 'alert-red' : 'green'}`}>
                <CloudRain size={27} />
              </div>
                ...
                <table className="locations-table">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td>
                        <span className={`risk-badge ${prediction?.risk_level === 'HIGH' ? 'high' : prediction?.risk_level === 'MEDIUM' ? 'moderate' : 'low'}`}>
                          {prediction?.risk_level || "..."}
                        </span>
                      </td>
                    </tr>
